    ...
    internal sealed class HealthCheckOperations : IDisposable
    {
        ...
        public HealthState Health
        {
            get
            {
                ...
                ServiceEventSource.Current.Message("HealthState" + Enum.GetName(typeof(HealthState), this._healthState));
        ...
        internal async Task<bool> ValidateServiceExistsAsync(Uri serviceName, Guid partition = default(Guid))
        {
            ...
            {
                //TODO: ServiceEventSource.Current.Exception(ex.Message, ex.GetType().Name, nameof(this.ValidateServiceExistsAsync));
                ServiceEventSource.Current.Message(ex.Message + ex.GetType().Name + nameof(this.ValidateServiceExistsAsync));
            }
        ...
        internal async Task EnumerateHealthChecksAsync()
        {
            ...
            using (ITransaction eTx = this._service.StateManager.CreateTransaction())
            {
                ...
                Data.IAsyncEnumerator<KeyValuePair<long, WatchdogScheduledItem>> ae =
                    (Data.IAsyncEnumerator<KeyValuePair<long, WatchdogScheduledItem>>)(await scheduleDict.CreateEnumerableAsync(eTx, EnumerationMode.Ordered)).GetAsyncEnumerator();
        ...
        public async Task<IList<HealthCheck>> GetHealthChecksAsync(string application = null, string service = null, Guid? partition = null)
        {
            ...
            {
                //TODO: ServiceEventSource.Current.Exception(ex.Message, ex.GetType().Name, nameof(this.GetHealthChecksAsync));
                ServiceEventSource.Current.Message(ex.Message + ex.GetType().Name + nameof(this.GetHealthChecksAsync));
            }
            catch (TimeoutException ex)
            {
                //TODO: ServiceEventSource.Current.Exception(ex.Message, ex.GetType().Name, nameof(this.GetHealthChecksAsync));
